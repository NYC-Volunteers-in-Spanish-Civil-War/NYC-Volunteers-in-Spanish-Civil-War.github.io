...
# Helper function for sorting documents by their id
sort_documents = lambda items: sorted(items, key = lambda k: [int(i) for i in k.split('_')])
...
for item in sorted(list(MASTER_DATA.keys())):
    if 'children' in MASTER_DATA[item]:
        MASTER_DATA[item]['children'] = sort_documents([sovdoc_data[child]['id'] for child in MASTER_DATA[item]['children']])
...
# Helper function for dealing with encoding
clean_name = lambda k : unescape(k).strip('.').replace('/', '*').replace('.html','')
...
for item in interbrigades_data:
    ...
    MASTER_DATA[id]['meta'] = {clean_name(k): [clean_name(v) for v in item[k]] for k in item}
...
META_TAGS = { **{'tags': list(META_TAGS.keys())},
              **{field:list(META_TAGS[field].keys()) for field in META_TAGS},
              **{tag:sort_documents(META_TAGS[field][tag]) for field in META_TAGS for tag in META_TAGS[field]}}
